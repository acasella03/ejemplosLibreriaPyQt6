...
class ModeloTabla(QAbstractTableModel):
    def __init__(self, tabla):
        ...
        self.cargar_datos() # Se cargan los datos de la base de datos
    # Métodos abstractos que deben implementarse
    def cargar_datos(self):
        conn = sqlite3.connect('baseDatos2.dat')
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM usuarios")
        self.tabla = cursor.fetchall()
        conn.close()
...
class VentanaPrincipal(QMainWindow):
    def __init__(self):
        ...
        super().__init__()
        self.setWindowTitle("Ejemplo QTableView y SQLite") # Establece el título de la ventana

        # Crea el modelo de datos de la tabla
        self.modelo = ModeloTabla(None)

        cajaV=QVBoxLayout() # Creación de un layout vertical
        self.tvwTabla = QTableView() # Creación de la vista de la tabla
        self.tvwTabla.setModel(self.modelo) # Configuración del modelo de datos de la tabla
